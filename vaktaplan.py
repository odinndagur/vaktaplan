...
class Vaktaplan:
    ...
    def pdf_to_df(cls,file: str) -> pd.DataFrame:
        ...
        concatenated_dfs = []
        num_pages = cls.get_num_pages(tables)
        for idx, offset in enumerate(num_pages):
            base_num = sum(num_pages[:min(idx-1,0)])
            concatenated_dfs.append(
                pd.concat(
                    processed_dfs[base_num:base_num+offset]
                )
            )
        # síðan join
